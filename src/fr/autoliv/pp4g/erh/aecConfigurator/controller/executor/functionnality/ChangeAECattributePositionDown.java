...
@details
	An action of the command pattern : change the AEC attribute position with the next
 */
...
public class ChangeAECattributePositionDown implements ICommand{

	@Override
	public void execute() {
		try {
			DialogStore.getInstance().getMainFrame().saveCalibration();
			AECcontroller.changeAECattributePosition(+1,false);
			DialogStore.getInstance().getMainFrame().saveCalibration();
