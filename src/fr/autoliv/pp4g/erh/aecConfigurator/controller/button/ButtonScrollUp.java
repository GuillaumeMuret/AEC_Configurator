...
@details
	Button use to scroll up the two files display
 */
...
public class ButtonScrollUp extends ModelButton{
	 ...
	 * @param mainFrame
	 */
	public ButtonScrollUp(MainFrame mainFrame){
		super(ConstantIcon.IC_SCROLL_UP,NativeKeyEvent.VC_UP,true,ModelButton.BUTTON_ICON,ConstantString.TOOL_TIP_BUTTON_SCROLL_UP);
	...
	synchronized public void buttonEvent(){
		if(this.mainFrame.getFrameState()==MainFrame.STATE_SHOW_DIFFERENCE){
			ControlStore.getInstance().getExecutor().executeCommand(IDCommand.SCROLL_UP);
		}
	}
}
