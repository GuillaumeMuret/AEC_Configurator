...
@details
	Button use to display the next page
 */
...
public class ButtonNextPage extends ModelButton{
	 ...
	 * @param mainFrame
	 */
	public ButtonNextPage(MainFrame mainFrame){
		super(ConstantIcon.IC_NEXT_PAGE,NativeKeyEvent.VC_RIGHT,true,ModelButton.BUTTON_ICON,ConstantString.TOOL_TIP_BUTTON_NEXT_PAGE);
	...
	synchronized public void buttonEvent(){
		if(this.mainFrame.getFrameState()==MainFrame.STATE_SHOW_DIFFERENCE){
			ControlStore.getInstance().getExecutor().executeCommand(IDCommand.DISPLAY_NEXT_PAGE);
		}
	}
}
